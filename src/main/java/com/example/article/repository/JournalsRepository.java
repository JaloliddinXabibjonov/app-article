...
public interface JournalsRepository extends JpaRepository<Journals, UUID> {
    ...
    @Query(value = "select release_number_of_this_year from journals where parent_id=?1 and date_publication=?2 and title=?3 order by created_at desc limit 1", nativeQuery = true)
    ...
    Date findDateOfPublicationByIdAndDeletedTrue(UUID id);
...
//    List<Journals> findAllByDeletedTrueAndActiveTrue
    ...
    List<Journals> findAllByDeletedTrueAndDatePublicationAndIdAndJournalsStatusOrDeletedTrueAndDatePublicationAndParentIdAndJournalsStatusOrderByDatePublication(int datePublication, UUID id, String journalsStatus, int datePublication2, UUID parentId, String journalsStatus2);
