...
//    Page<Article>findAllByChecked(String checked, Pageable pageable);
    ...
    @Query(value = "select  * from article a  inner join journals j on j.id=?1  ", nativeQuery = true)
    List<Article> journalArticles(UUID id);

@Query(value = "select  * from article a  inner join journals j on j.id=?1 and a.article_status_name='PUBLISHED' and a.journals_active=true", nativeQuery = true)
    ...
    @Query(value = "select a.* from article a inner join article_authors au  on article_status_name=?1  and authors_id=?2",nativeQuery = true)
    List<Article> findAllByArticleStatusNameAndAuthorsCode(ArticleStatusName articleStatusName, Integer code);
